...
def transfer(
    ...
    handler_instances = {
        handler_type: handler_type(model, target_model, calibrate_dataset)
        for handler_type in OrderedDict.fromkeys(pass_type.values())
        if handler_type is not AMMConv2dTransferHandler  # Skip AMMConv2dTransferHandler
    }
    filtered_pass_type = {k: v for k, v in pass_type.items() if v is not AMMConv2dTransferHandler}
    ...
    handlers = {
        key: handler_instances[value]
        for key, value in filtered_pass_type.items()
    }
    ...
    for name, module in model.named_modules():
        ...
        getattr(handler, func)(*args) # AMMLinearTransferHandler.transfer
