...
from operations.im2col import unfold
from operations.utils import calc_output_shape
...
class AMMConv2d(nn.Module):
    ...
    @staticmethod
    def _quantize_lut(lut: torch.Tensor, bit_width: int = 8) -> QuantizedTensor:
        # Quantize weight to a specified bit-width range
        a = lut.min()
        b = lut.max()
        ...
        z = torch.zeros_like(a).to(torch.float32)
        s = max_abs / (max_val - z)
